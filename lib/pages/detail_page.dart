        ...
        background: _controller != null
            ...
            : Image.network(
                ...
                errorBuilder: (context, error, stackTrace) =>
                    const Center(child: Icon(Icons.error, color: OtsuColor.grey)),
              ),
      ),
    );
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    _buildStatChip("Rank", "#${widget.anime.rank ?? 'N/A'}", OtsuColor.secondary),
                    const SizedBox(width: 8),
                    _buildStatChip("Popularity", "#${widget.anime.popularity ?? 'N/A'}", OtsuColor.secondary),
                  ],
                ),
              ],
            ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: OtsuColor.primary)),
        ],
      ),
    );
  }
        ...
        children: List<Widget>.generate(statuses.length, (int index) {
          return ChoiceChip(
            ...
            shape: RoundedRectangleBorder(
              ...
              side: BorderSide(
                color: _selectedStatusIndex == index + 1 ? Colors.transparent : OtsuColor.grey.withAlpha(128),
              ),
            ),
            onSelected: (bool selected) {
              setState(() {
                _selectedStatusIndex = selected ? index + 1 : 0;
              });
            },
          );
        }).toList(),
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              widget.anime.synopsis.isEmpty ? "No synopsis available." : widget.anime.synopsis,
              style: const TextStyle(
                fontSize: 14,
        ...
        children: [
          ...
          GridView.count(
            ...
            children: [
              ...
              _buildInfoTile("Season", "${widget.anime.season?.capitalize() ?? 'N/A'} ${widget.anime.year ?? ''}"),
              _buildInfoTile("Members", widget.anime.members?.toString() ?? 'N/A'),
              _buildInfoTile("Favorites", widget.anime.favorites?.toString() ?? 'N/A'),
            ],
          ),
          const SizedBox(height: 12),
          _buildInfoTile("Studios", widget.anime.studios.join(', ').isEmpty ? 'N/A' : widget.anime.studios.join(', ')),
          const SizedBox(height: 8),
          _buildInfoTile("Producers", widget.anime.producers.join(', ').isEmpty ? 'N/A' : widget.anime.producers.join(', ')),
        ],
      ),
    );
  }
